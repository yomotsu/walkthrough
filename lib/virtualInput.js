...
virtualInput.Joystick = function ( $container, size, params ) {
  ...
  this.pointerId = virtualInput.getUUID();
  this.isActive = false;
  ...
  var onbuttondown = function ( event ) {
    ...
    that.isActive = true;
    $win.on( virtualInput._move, onbuttonmove );
    $win.on( virtualInput._end,  onbuttonup );
    var x = (   ( event.originalEvent.clientX - that.offset.left ) - that.halfWidth ) / that.halfWidth * 2;
    var y = ( - ( event.originalEvent.clientY - that.offset.top  ) + that.halfWidth ) / that.halfWidth * 2;
  ...
  var onbuttonup = function ( event ) {
    ...
    $win.off( virtualInput._move, onbuttonmove );
    $win.off( virtualInput._end,  onbuttonup );

  }

  this.setCSSPosition( 0, 0 );
  this.$all.on( virtualInput._start, onbuttondown );
